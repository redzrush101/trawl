...
def _make_client(ctx, source):
    ...
    if source == "ablib":
        return ablib_src.Client(**kwargs)
    return eshia_src.Client(**kwargs)


def _handle_error(e, json_mode, label="Error"):
    if json_mode:
        return {"error": str(e)}
...
def _search_ablib(
    ...
    author, title, publisher, lang, cat, pdf, json_mode,
):
    json_dict = None
    raw_result = None
    filters = {
    ...
    try:
        with _make_client(ctx, "ablib") as c:
            raw_result = c.search(query=query, page=page, per_page=per_page, **filters)
        if exclude:
        ...
        json_dict = _handle_error(e, json_mode, "## Source: ablibrary")
    return json_dict, raw_result


def _search_eshia(ctx, query, page, book_id, category, exclude, arabic, json_mode):
    json_dict = None
    all_results = []
    raw_cp = 0
    raw_total = 0
    try:
        group_key = None
        if category:
            with _make_client(ctx, "eshia") as c2:
                group_key = c2.resolve_group_key(category)
        with _make_client(ctx, "eshia") as c:
            if book_id:
        ...
        json_dict = _handle_error(e, json_mode, "## Source: eshia")
...
@click.pass_context
def search(
    ctx, query, source, page, per_page, exclude, save, save_format,
    ...
    if source in ("both", "ablib"):
        jd, ablib_raw = _search_ablib(
            ctx, query, page, per_page, exclude, author, title, publisher, lang, cat, pdf, json_mode)
        if json_mode:
            json_data["sources"]["ablibrary"] = jd

    if source in ("both", "eshia"):
        jd, eshia_raw, eshia_cp, eshia_total = _search_eshia(
            ctx, query, page, book_id, category, exclude, arabic, json_mode)
        if json_mode:
            json_data["sources"]["eshia"] = jd

    if json_mode:
        json_output = j.dumps(json_data, indent=2, default=str, ensure_ascii=False)
        if save:
            Path(save).write_text(json_output)
            click.echo(json_output, nl=False)
        else:
...
@click.pass_context
def read(ctx, book_id, pages, source, volume, max_tokens, parallel, workers, images, chains):
    """Read book pages."""
    json_mode = ctx.obj["json"]

    truncate_tokens = max_tokens
                ...
                with ThreadPoolExecutor(max_workers=workers) as ex:
                    ...
                    for f in _progress(as_completed(futures), desc="Fetching pages", total=len(page_nums)):
                        ...
                        try:
                            ...
                            _handle_error(e, json_mode, f"Error on page {pn}")
            else:
                with _make_client(ctx, "eshia") as c:
...
@click.pass_context
def ablib_search_text(ctx, book_id, query, max_pages, regex):
    """Search text within book pages (client-side)."""
    json_mode = ctx.obj["json"]
    try:
        with _make_client(ctx, "ablib") as c:
            matching = c.search_in_text(book_id, query, max_pages=max_pages, regex=regex)
        if json_mode:
...
def _download_page(page, book_id, volume, c, out, json_mode):
...
def download(ctx, book_id, volume, pages, output, parallel, workers):
    ...
    try:
        if parallel and len(page_nums) > 1:
            with ThreadPoolExecutor(max_workers=workers) as ex:
                futures = {
                    ex.submit(_download_page, p, book_id, volume, eshia_src.Client(**cfg), out, json_mode): p
                    for p in page_nums
                }
            ...
            with _make_client(ctx, "eshia") as c:
                for p in _progress(page_nums, desc="Downloading"):
                    r = _download_page(p, book_id, volume, c, out, json_mode)
                    if json_mode and r is not None:
                        results.append(r)
    except Exception as e:
        _handle_error(e, json_mode)
...
def _narrator_ablib(ctx, name, json_mode):
    ...
    try:
        ...
        result = _handle_error(e, json_mode)
    return result


def _narrator_eshia(ctx, name, json_mode):
    result = None
    try:
        with _make_client(ctx, "eshia") as c:
...
def narrator(ctx, name):
    ...
    eshia_data = _narrator_eshia(ctx, name, json_mode)

    if json_mode:
        data = {"name": name, "sources": {"ablibrary": ablib_data, "eshia": eshia_data}}
        click.echo(j.dumps(data, indent=2, default=str, ensure_ascii=False))
